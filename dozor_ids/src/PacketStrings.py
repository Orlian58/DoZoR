...
def packetStringJSON(pkt):
    out = {}
    if (IP in pkt):
        out['IP'] = ipStringJSON(pkt[IP])
    elif (IPv6 in pkt):
        pass
    if (TCP in pkt):
        out['TCP'] = tcpStringJSON(pkt[TCP])
        out['TCP Payload'] = payloadStringJSON(pkt[TCP])
    elif (UDP in pkt):
        out['UDP'] = udpStringJSON(pkt[UDP])
        out['UDP Payload'] = payloadStringJSON(pkt[UDP])
    return out
